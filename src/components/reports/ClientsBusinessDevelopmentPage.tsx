...
export default function ClientsBusinessDevelopmentPage({ userRole }: { userRole: UserRole }) {
  ...
  const tableSortValueOf = (row: any, key: string): unknown => {
    if (detail.kind === 'experience') {
      switch (key) {
        case 'record': return row.title || row.subject || row.feedbackType || row.complaintType || row.description || 'Record';
        case 'client': return row.clientName || row.prospect?.clientName || row.matterId || '';
        case 'type': return row.feedbackType || row.type || row.complaintType || '';
        case 'status': return row.status || '';
        case 'date': return row.createdAt || row.sentAt || row.dateReceived || '';
        default: return row.title || row.subject || row.clientName || '';
      }
    }
    if (clientView(view)) {
      ...
      switch (key) {
        ...
        case 'clientStatus': return client.matters.some((matter) => normalize(matter.status) !== 'closed') ? 'Active' : 'Completed';
        case 'activeTotal': return client.matters.filter((matter) => normalize(matter.status) !== 'closed').length;
        case 'billed': return client.billed;
        case 'collected': return client.collected;
        case 'outstanding': return client.outstanding;
        default: return client.name;
      }
    }
    const prospect = row as Prospect;
    switch (key) {
      case 'prospect': return prospect.clientName || '';
      case 'stage': return prospect.stage || '';
      case 'practice': return prospect.legalServicePath?.at(-1)?.label || '';
      case 'fee': return amount(prospect.estimatedFeeValue);
      case 'referral': return prospect.referralSource || '';
      case 'partner': return typeof prospect.responsiblePartner === 'object' ? prospect.responsiblePartner?.name || '' : prospect.responsiblePartner || '';
      default: return prospect.clientName || '';
    }
  };
  const sortedTableRows = sortRows(tableRows as any[], sortKey, sortDir, (row) => tableSortValueOf(row, sortKey));
  const pages = Math.max(1, Math.ceil(sortedTableRows.length / PAGE_SIZE));
  const visibleRows = sortedTableRows.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
  useEffect(() => setPage(1), [search, view, range, sortKey, sortDir]);
  useEffect(() => setPage((currentPage) => Math.min(currentPage, pages)), [pages]);

  const exportColumns = detail.kind === 'experience' ? [
    { label: 'Record', value: (r: any) => String(tableSortValueOf(r, 'record') ?? '') },
    { label: 'Client / Matter', value: (r: any) => String(tableSortValueOf(r, 'client') ?? '') },
    { label: 'Type', value: (r: any) => String(tableSortValueOf(r, 'type') ?? '') },
    { label: 'Status', value: (r: any) => String(tableSortValueOf(r, 'status') ?? '') },
    { label: 'Date', value: (r: any) => String(tableSortValueOf(r, 'date') ?? '') },
  ] : clientView(view) ? [
    { label: 'Client', value: (r: any) => String(tableSortValueOf(r, 'client') ?? '') },
    { label: 'Status', value: (r: any) => String(tableSortValueOf(r, 'clientStatus') ?? '') },
    { label: 'Active Matters', value: (r: any) => Number(tableSortValueOf(r, 'activeTotal') ?? 0) },
    { label: 'Total Matters', value: (r: any) => ((r as ClientRow).matters || []).length },
    { label: 'Billed (RWF)', value: (r: any) => Number(tableSortValueOf(r, 'billed') ?? 0) },
    { label: 'Collected (RWF)', value: (r: any) => Number(tableSortValueOf(r, 'collected') ?? 0) },
    { label: 'Outstanding (RWF)', value: (r: any) => Number(tableSortValueOf(r, 'outstanding') ?? 0) },
  ] : [
    { label: 'Prospect', value: (r: any) => String(tableSortValueOf(r, 'prospect') ?? '') },
    { label: 'Prospect No', value: (r: any) => String((r as Prospect).prospectNo || '') },
    { label: 'Stage', value: (r: any) => String(tableSortValueOf(r, 'stage') ?? '') },
    { label: 'Practice Area', value: (r: any) => String(tableSortValueOf(r, 'practice') ?? '') },
    { label: 'Estimated Fee (RWF)', value: (r: any) => Number(tableSortValueOf(r, 'fee') ?? 0) },
    { label: 'Referral Source', value: (r: any) => String(tableSortValueOf(r, 'referral') ?? '') },
    { label: 'Responsible Partner', value: (r: any) => String(tableSortValueOf(r, 'partner') ?? '') },
  ];
