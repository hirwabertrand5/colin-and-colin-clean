...
export default function BillingFinancePage({ view, userRole }: { view: BillingFinanceView; userRole: UserRole }) {
  ...
  if (loading) return <div className="py-12 text-center text-gray-500">Loading finance data...</div>;
...
function FinanceTable({ view, cases, expenses, report, invoices }: { view: BillingFinanceView; cases: CaseData[]; expenses: PettyCashExpense[]; report: FirmReportResponse | null; invoices: InvoiceWithCase[] }) {
  ...
  return <div className="overflow-hidden rounded-lg border border-gray-200 bg-white"><table className="min-w-full text-left text-sm"><thead className="bg-gray-50 text-xs uppercase tracking-wide text-gray-500"><tr><th className="px-5 py-3">#</th><th className="px-5 py-3">Record</th><th className="px-5 py-3">Details</th><th className="px-5 py-3">Value</th></tr></thead><tbody>{rows.length === 0 ? <tr><td colSpan={4} className="px-5 py-10 text-center text-gray-500">No supporting records are available.</td></tr> : rows.map((row, index) => <tr key={`${row.label}-${index}`} className="border-t border-gray-100"><td className="px-5 py-4 text-gray-500">{index + 1}</td><td className="px-5 py-4 font-medium text-gray-900">{row.label || 'Unavailable'}</td><td className="px-5 py-4 text-gray-600">{row.detail || 'Unavailable'}</td><td className="px-5 py-4 font-semibold text-gray-900">{row.value}</td></tr>)}</tbody></table></div>;
}