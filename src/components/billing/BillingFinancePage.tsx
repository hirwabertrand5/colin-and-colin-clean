...
function FinanceTable({ view, cases, expenses, report, invoices }: { view: BillingFinanceView; cases: CaseData[]; expenses: PettyCashExpense[]; report: FirmReportResponse | null; invoices: InvoiceWithCase[] }) {
  const rows = view === 'expenses' || view === 'expense-direct-costs' || view === 'expense-operating' ? expenses.filter((expense) => view === 'expense-direct-costs' ? expense.chargeType === 'client' : view === 'expense-operating' ? expense.chargeType !== 'client' : true).map((expense) => ({ label: expense.title, detail: `${expense.date} • ${expense.category || 'Unclassified'} • ${expense.chargeType === 'client' ? 'Client' : 'Internal'}`, value: money(Math.max(0, amount(expense.amount) - amount(expense.refundAmount))), date: expense.date, timestamp: expense.createdAt, actor: expense.createdByName, reference: expense.receiptRef || expense.caseNoSnapshot || 'No reference' })) : view === 'staff-profitability' || view === 'fee-earned' || view === 'by-staff' ? (report?.team || []).map((member) => ({ label: member.name, detail: member.role, value: money(member.earnedFees || member.revenueAttributed || 0), date: 'Period total', timestamp: undefined, actor: member.name, reference: `${member.tasksCompleted} tasks` })) : cases.map((matter) => { const matterInvoices = invoices.filter((invoice) => String(invoice.caseId) === String(matter._id)); const matterBilled = matterInvoices.reduce((sum, invoice) => sum + amount(invoice.amount), 0); const matterCollected = matterInvoices.filter((invoice) => invoice.status === 'Paid').reduce((sum, invoice) => sum + amount(invoice.amount), 0); return { label: matter.caseNo, detail: matter.parties, value: money(view === 'contract-value' ? Math.max(amount(matter.workflowProgress?.plannedValue?.amount), amount(matter.budget)) : view === 'total-collected' ? matterCollected : matterBilled), date: matter.updatedAt || matter.createdAt || 'Date unavailable', timestamp: matter.updatedAt || matter.createdAt, actor: 'Actor unavailable', reference: matter.status || 'Status unavailable' }; });
  const [page, setPage] = useState(1);
  const totalPages = Math.max(1, Math.ceil(rows.length / 10));
  const visibleRows = rows.slice((page - 1) * 10, page * 10);
  useEffect(() => setPage(1), [view, rows.length]);

  return <div className="overflow-hidden rounded-lg border border-gray-200 bg-white"><div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-gray-50 text-xs uppercase tracking-wide text-gray-500"><tr><th className="px-5 py-3">#</th><th className="px-5 py-3">Record</th><th className="px-5 py-3">Details</th><th className="px-5 py-3">Value</th><th className="px-5 py-3">Date</th><th className="px-5 py-3">Timestamp</th><th className="px-5 py-3">Recorded By</th><th className="px-5 py-3">Reference</th></tr></thead><tbody>{rows.length === 0 ? <tr><td colSpan={8} className="px-5 py-10 text-center text-gray-500">No supporting records are available.</td></tr> : visibleRows.map((row, index) => <tr key={`${row.label}-${index}`} className="border-t border-gray-100"><td className="px-5 py-4 text-gray-500">{(page - 1) * 10 + index + 1}</td><td className="px-5 py-4 font-medium text-gray-900">{row.label || 'Unavailable'}</td><td className="px-5 py-4 text-gray-600">{row.detail || 'Unavailable'}</td><td className="px-5 py-4 font-semibold text-gray-900">{row.value}</td><td className="px-5 py-4">{row.date}</td><td className="px-5 py-4">{row.timestamp ? new Date(row.timestamp).toLocaleString() : 'Timestamp unavailable'}</td><td className="px-5 py-4">{row.actor || 'Actor unavailable'}</td><td className="px-5 py-4 text-gray-600">{row.reference}</td></tr>)}</tbody></table></div>{rows.length > 0 && <Pagination page={page} pages={totalPages} total={rows.length} onChange={setPage} />}</div>;
}