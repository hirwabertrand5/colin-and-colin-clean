...
export default function ClosedCases({ userRole }: ClosedCasesProps) {
  ...
  const paginatedCases = filteredSortedCases.slice((currentPage - 1) * CASES_PER_PAGE, currentPage * CASES_PER_PAGE);
    ...
    <div>
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-semibold text-gray-900 mb-1">Closed Matters</h1>
            <p className="text-gray-600">Historical and closed matters (complete case history preserved)</p>
          </div>

          {/* no create button for closed matters */}
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        ...
        {loading && <div className="text-center py-12 text-gray-500">Loading closed matters...</div>}

        {!loading && filteredSortedCases.length === 0 && (
          <div className="text-center py-12">
            <Briefcase className="w-12 h-12 text-gray-300 mx-auto mb-3" />
            <p className="text-gray-500">No closed matters found</p>
          </div>
        )}

        {!loading && filteredSortedCases.length > 0 && (
          <div className="flex items-center justify-between border-t border-gray-200 px-6 py-4">
            <p className="text-sm text-gray-600">Showing {(currentPage - 1) * CASES_PER_PAGE + 1}-{Math.min(currentPage * CASES_PER_PAGE, filteredSortedCases.length)} of {filteredSortedCases.length} cases</p>

            <div className="flex items-center gap-2">
              <button type="button" onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50">Previous</button>
              <span className="text-sm text-gray-600">Page {currentPage} of {totalPages}</span>
              <button type="button" onClick={() => setCurrentPage((prev) => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50">Next</button>
            </div>
          </div>
        )}
      </div>
