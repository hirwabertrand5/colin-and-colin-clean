...
export default function DeadlineManagementPage({ view }: { view: DeadlineView }) {
  ...
  const deadlineSortValueOf = (event: FirmCalendarEvent, key: string): unknown => {
    switch (key) {
      case 'deadline':
        return event.title || '';
      case 'matter':
        return `${caseMap.get(event.caseId)?.caseNo || ''} ${caseMap.get(event.caseId)?.parties || ''}`;
      case 'type':
        return event.type || '';
      case 'dueDate':
        return resolveDeadlineDateTime(`${event.date}T${event.time || '12:00'}`)?.getTime() || 0;
      case 'status':
        return dateKey(event.date) < todayKey() ? 1 : 0;
      default:
        return resolveDeadlineDateTime(`${event.date}T${event.time || '12:00'}`)?.getTime() || 0;
    }
  };
  const sorted = useMemo(
    () => sortRows(filtered, sortKey, sortDir, (event) => deadlineSortValueOf(event, sortKey)),
    [filtered, sortDir, sortKey],
  );

  const totalPages = Math.max(1, Math.ceil(sorted.length / PAGE_SIZE));
  const paginated = sorted.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
  useEffect(() => setPage(1), [horizon, search, sortDir, sortKey, view]);
