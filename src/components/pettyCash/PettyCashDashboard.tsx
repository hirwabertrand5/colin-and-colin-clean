...
export default function PettyCashDashboard() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError('');
      const [active, allCases, funds, allUsers] = await Promise.all([
        getActivePettyCashFund(),
        getAllCases().catch(() => []),
  ...
  const handleAddExpense = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addExpenseToFund(fund._id, {
        ...
        receivedByUserId: expenseForm.receivedByUserId || undefined,
  ...
  const getUserName = (userId?: string) => {
    if (!userId) return '';
    return users.find((user) => String(user._id) === String(userId))?.name || '';
  };

  const getReceivedByLabel = (expense: PettyCashExpense) => {
    if (expense.receivedByName) return expense.receivedByName;
    return getUserName(expense.receivedByUserId) || '—';
  };
  ...
  const buildLedgerExportRows = () =>
    workspaceExpenses.map((expense, index) => [
      index + 1,
      expense.date,
      expense.itemDescription || expense.title || '—',
      getReceivedByLabel(expense),
      expense.createdByName || '—',
      Number(expense.amount) || 0,
      expense.matterId || expense.caseNoSnapshot || '—',
    ]);

  const exportLedgerToExcel = async () => {
    if (!workspaceFund || workspaceExpenses.length === 0) return;

    await downloadWorkbook(
      `${workspaceFund.name.replace(/[^\w\s-]/g, '').trim().replace(/\s+/g, '_') || 'petty_cash_ledger'}.xlsx`,
      [
        {
          title: 'Fund Ledger',
          sections: [
            {
              title: `${workspaceFund.name} Ledger`,
              headers: ['#', 'Date', 'Item Description', 'Received By', 'Logged By', 'Amount (RWF)', 'Linked Matter / File ID'],
              rows: buildLedgerExportRows(),
              currencyColumns: [6],
              centerColumns: [1, 2, 3, 4, 5, 7],
            },
          ],
        },
      ]
              ...
              <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={workspaceExpenses.length === 0}
                    className="inline-flex items-center gap-2 rounded-full border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
                  >
                ...
                <table className="min-w-full text-sm">
                  ...
                  <tbody>
                    {workspaceExpenses.length === 0 ? (
                      <tr>
                        <td colSpan={8} className="px-3 py-6 text-center text-slate-500 dark:text-slate-400">No expenses recorded in this fund cycle.</td>
                      </tr>
                    ) : (
                      workspaceExpenses.map((expense, index) => (
                        <tr key={expense._id} className="border-t border-slate-200 text-slate-700 dark:border-slate-700 dark:text-slate-300">
                          ...
                          <td className="px-3 py-3">{getReceivedByLabel(expense)}</td>
          ...
          <div className="bg-white rounded-lg max-w-lg w-full flex flex-col" style={{ maxHeight: '90vh' }}>
            ...
            <form onSubmit={handleAddExpense} className="flex-1 overflow-y-auto space-y-4 p-6">
              ...
              <div>
                ...
                <select
                  value={expenseForm.receivedByUserId}
                  onChange={(e) => setExpenseForm((p) => ({ ...p, receivedByUserId: e.target.value }))}
                ...
                <p className="mt-1 text-xs text-gray-500">Choose the staff member who received or processed the cash.</p>
              </div>
